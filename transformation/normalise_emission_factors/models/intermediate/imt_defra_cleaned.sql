...
defra_intermediate as (
    select
        id,
        case
            when sub_category in ('Hotel stay')
            ...
            when coalesce(sub_category_new,sub_category) in ('Motorbike')
                then factor_name || sub_category
            when coalesce(sub_category_new,sub_category) in ('Heavy Goods Vehicles','Heavy Goods Vehicles - Refrigerated','Vans','WTT- Heavy Goods Vehicles','WTT- Heavy Goods Vehicles - Refrigerated','WTT- Vans' )
...
                then coalesce(sub_category_new,sub_category) || ' -' || factor_name
            when coalesce(sub_category_new,sub_category) in ('WTT- biomass','WTT- heat and steam', 'WTT- biogas','WTT- fuels', 'WTT- Bus','WTT- Taxis','WTT- Rail','WTT- Freight Flights','WTT- Sea Tanker','WTT- Cargo Ship')
